...
def run_rf(folds, header, class_index, ntree, classes, k):
    total_accuracy = 0
    total_precision = 0
    total_recall = 0
    total_F1 = 0
    for i in folds:
        ...
        for tree in range(ntree):
            ...
            curtree = DecisionTree(bstree, header[class_index], 15, header, class_index)
...
def graph_accuracy(folds, header, class_index, classes, k):
    y = []
    x = [1, 5, 10, 20, 30]
    for ntree in x:
        y_value = run_rf(folds, header, class_index, ntree, classes, k)[0]
        print(y_value)
        y.append(y_value)
    plt.plot(x, y)
